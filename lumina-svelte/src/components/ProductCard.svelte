...
<div
  class="product-card relative flex h-full w-full cursor-pointer flex-col rounded-[2.5rem] border border-neutral-100 bg-white text-left shadow-sm"
  role="button"
  tabindex="0"
...
>
  ...
  <div class="flex flex-1 flex-col p-6 md:p-8">
    <div class="mb-2 flex items-center gap-2 text-xs font-bold text-amber-500">
      ...
      <span class="font-medium text-neutral-400">({product.reviewCount})</span>
    </div>
    ...
    <div class="mt-auto flex items-end justify-between border-t border-neutral-100/60 pt-4">
      <div class="flex flex-col">
        {#if product.originalPrice}
          <span class="text-xs font-medium text-neutral-400 line-through">
      ...
      <button
        type="button"
        class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-neutral-900 text-white shadow-md transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:bg-neutral-300"
