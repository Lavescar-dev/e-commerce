<script>
  ...
  import { fallbackSvg, products } from '../data/products.js';
...
{#if product}
  ...
  >
    <div class="modal-pop flex max-h-[95vh] w-full max-w-5xl flex-col overflow-hidden rounded-[3rem] bg-white shadow-2xl md:flex-row">
      <div class="relative flex items-center justify-center overflow-hidden bg-neutral-50 md:w-1/2">
        {#if !imageLoaded}
      ...
      <div class="flex flex-col overflow-auto p-10 md:w-1/2 md:p-14">
        <div class="flex items-start justify-between">
          ...
          <button type="button" class="text-neutral-300 transition hover:text-neutral-900" onclick={onClose} aria-label="Kapat">
        ...
        <div class="mt-4 flex items-center gap-3">
          ...
          <span class="text-sm font-bold text-neutral-900">{product.rating.toFixed(1)}</span>
          <span class="text-xs font-medium text-neutral-400">({product.reviewCount} yorum)</span>
        </div>

        <div class="mt-8 flex items-end gap-3">
          {#if product.originalPrice}
            <div class="text-4xl font-black text-rose-600">{product.price} ₺</div>
            <div class="text-lg font-medium text-neutral-400 line-through">{product.originalPrice} ₺</div>
          {:else}
            <div class="text-4xl font-black text-neutral-900">{product.price} ₺</div>
          {/if}
        </div>

        <div class="mt-4 flex items-center gap-2 text-xs font-black uppercase tracking-widest">
          {#if stockState === 'out'}
            <span class="rounded-full bg-neutral-100 px-3 py-1.5 text-neutral-600">Tükendi</span>
          {:else if stockState === 'low'}
            <span class="rounded-full bg-amber-50 px-3 py-1.5 text-amber-700">
              Son {product.stock} adet
            </span>
          {:else}
            <span class="rounded-full bg-emerald-50 px-3 py-1.5 text-emerald-700">Stokta Var</span>
          {/if}
        </div>

        <div class="my-10 h-px w-20 bg-neutral-200"></div>
        <p class="text-base leading-relaxed font-medium text-neutral-500">
          {product.desc}
        </p>

        <div class="mt-8 grid grid-cols-3 gap-3 text-xs">
          <div class="flex flex-col items-center gap-2 rounded-2xl border border-neutral-100 p-4 text-center">
            <span class="text-emerald-700"><Icon name="truck" size={20} /></span>
            <span class="font-bold text-neutral-700">Hızlı Kargo</span>
          </div>
          <div class="flex flex-col items-center gap-2 rounded-2xl border border-neutral-100 p-4 text-center">
            <span class="text-emerald-700"><Icon name="leaf" size={20} /></span>
            <span class="font-bold text-neutral-700">Doğal İçerik</span>
          </div>
          <div class="flex flex-col items-center gap-2 rounded-2xl border border-neutral-100 p-4 text-center">
            <span class="text-emerald-700"><Icon name="refresh" size={20} /></span>
            <span class="font-bold text-neutral-700">30 Gün İade</span>
          </div>
        </div>
        ...
        <div class="mt-auto flex gap-3 pt-10">
          <button
            type="button"
            class="flex flex-1 items-center justify-center gap-x-3 rounded-[2rem] bg-emerald-800 py-5 text-base font-bold text-white shadow-xl shadow-emerald-900/20 transition hover:bg-emerald-900 disabled:cursor-not-allowed disabled:bg-neutral-300 disabled:shadow-none"
          ...
          <button
            type="button"
            class="wishlist-btn flex h-[68px] w-[68px] items-center justify-center rounded-[2rem] border border-neutral-200 transition hover:border-rose-200"
        ...
        {#if relatedProducts.length > 0}
          <div class="mt-12 border-t border-neutral-100 pt-8">
            <div class="mb-5 text-[10px] font-black uppercase tracking-[0.3em] text-emerald-700">
              Benzer Ürünler
            </div>
            <div class="grid grid-cols-2 gap-3 md:grid-cols-4">
              {#each relatedProducts as rp (rp.id)}
                <button
                  type="button"
                  class="group flex flex-col gap-2 rounded-2xl border border-neutral-100 bg-white p-2 text-left transition hover:border-emerald-200 hover:shadow-md"
                ...
                >
                  ...
                  <div class="px-1">
                    <div class="line-clamp-1 text-[11px] font-bold text-neutral-900">{rp.name}</div>
                    <div class="text-[11px] font-black text-emerald-700">{rp.price} ₺</div>
                  </div>
                </button>
              {/each}
            </div>
